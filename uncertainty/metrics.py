...
class selective_metrics():
    ...
    def plot_ROC_and_RC(self, aurc = False, auroc = True, *args):
        ...
        ax1.legend()
        ax1.xticks(fontsize=self.TICKS_FONTSIZE)
        ax1.yticks(fontsize=self.TICKS_FONTSIZE)
        ax1.grid()
        #self.fix_scale = False
        for name,(fpr,tpr) in self.ROC.items():
            label = name+f' | AUROC = {auc(fpr,tpr)}' if auroc else name
        ...
        ax2.legend()
        ax2.xticks(fontsize=self.TICKS_FONTSIZE)
        ax2.yticks(fontsize=self.TICKS_FONTSIZE)
        ax2.grid()
